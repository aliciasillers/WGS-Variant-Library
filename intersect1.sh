...
file1=$(ls MutectOut/*bb.filt.vcf | sed -n ${SLURM_ARRAY_TASK_ID}p)
file2=$(ls MutectOut/*bwa.filt.vcf | sed -n ${SLURM_ARRAY_TASK_ID}p)
prefix=$(ls MutectOut/*bb.filt.vcf | sed -n ${SLURM_ARRAY_TASK_ID}p | awk -F'[b]' '{print $1}')
